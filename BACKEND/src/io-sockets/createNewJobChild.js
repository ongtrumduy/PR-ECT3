    ...
    socket.on("create-new-job-child", data => {
      ...
      switch (data.jobChildOptionName) {
        case "lĩnh vực":
          ...
          return socket.emit("return-list-job-child", newJobTypeChild);
        case "vị trí":
          ...
          return socket.emit("return-list-job-child", newJobPositionChild);
        case "hoạt động":
          ...
          return socket.emit("return-list-job-child", newJobActivityChild);
      }
    });
  });
};
